...
suspend fun seedDefaultCategoriesIfNeeded(db: AppDatabase, userId: String) {
    ...
    val defaults = listOf(
        Category(userId = userId, name = "Groceries", color = "#FF29B6B2", budgetLimit = 3000.0),
        Category(userId = userId, name = "Transport", color = "#FFD97743", budgetLimit = 1500.0),
        Category(userId = userId, name = "Entertainment", color = "#FF9B3A62", budgetLimit = 500.0),
        Category(userId = userId, name = "Utilities", color = "#FFFF7A1A", budgetLimit = 2000.0)
    )
